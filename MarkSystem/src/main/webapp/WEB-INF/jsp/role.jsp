...
<title>Rimiedu_MSystem|role</title>
									...
									<div id="example1_filter" class="dataTables_filter">
										...
										<div class="col-sm-6"
											...
											<div class="btn-group">
												...
												<c:if test="${fn:contains(functions,'增加职位')}">
												<button type="button" class="btn btn-info " data-toggle="tooltip" data-placement="top" title="添加职位"
													onclick="tjzw()">
											...
											<tbody>
												...
												<c:forEach var="it" items="${rolePage}">
													<tr role="row" class="odd">
														...
														<c:if test="${fn:contains(functions,'查看职位')}">
														<span
															...
															aria-hidden="true" data-toggle="tooltip" data-placement="right" title="编辑"
															onclick="bj('${it.roleName}')" style="color: #00c0ef;"></span>
															
															</c:if>
															<%-- 
															<c:if test="${fn:contains(functions,'删除职位')}">
...
<div class="row">
  ...
  //搜索框赋值
	...
	var xsys = 6;// 页面显示多少页
	var yuan;
	var dqy = "<a aria-controls='example1' data-dt-idx='1' tabindex='0' href=role?currentPageName="+ dangqianye + "&value=${page.getName()}>" + dangqianye + "</a>";
	$("#dqy").append(dqy);
	for (var i = 1; i <= yeshu; i++) {
		if (yeshu < xsys) {
			if (i < dangqianye) {
				yuan = "<a aria-controls='example1' data-dt-idx='" + i
						+ "' tabindex='0' href=role?currentPageName=" + i + "&value=${page.getName()}>" + i
						+ "</a>";
	...
	function syy() {
		...
		location.href="role?value=${page.getName()}"+"&currentPageName="+ dangqianye;
	}
	// 下一页
	function xyy() {
		dangqianye += 1;
